...
//  gisuchat
//
//  Created by GISU KIM on 2016-12-17.
...
class ChatMessageCell: UICollectionViewCell, UITextFieldDelegate {
    ...
    func handleZoomVideo(tapGesture: UITapGestureRecognizer){
        
    }
    
    var count = 0
    
    func playerStopAndRestart(){
        if count % 2 == 0 {
            player?.pause()
        }else {
            player?.play()
    ...
    func handlePlay(){
        if let videoUrlString = message?.videoUrl, let url = URL(string: videoUrlString) {
            ...
            //spining icone when you click the button
